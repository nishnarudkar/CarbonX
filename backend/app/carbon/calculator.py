...
def _transport_annual_kg(t: TransportInput) -> float:
    """Calculate the annual transport footprint in kg CO2e."""
...
def _consumption_annual_kg(c: ConsumptionInput) -> float:
    """Calculate the annual goods and waste footprint in kg CO2e."""
...
def calculate_footprint(data: CarbonInput) -> FootprintResult:
    ...
    return FootprintResult(
        ...
        total_annual_tonnes=round(total / _KG_PER_TONNE, 3),
        comparison=comparison,
    )
