...
def _transport_recommendation(data: CarbonInput, amount: float) -> Recommendation | None:
    """Recommend reduction actions for the transport category based on travel habits."""
    t = data.transport
    flights_km = (
...
def _home_recommendation(amount: float) -> Recommendation | None:
    """Recommend reduction actions for home energy footprint."""
    if amount <= 0:
        return None
    return Recommendation(
        category="home",
...
def _diet_recommendation(data: CarbonInput) -> Recommendation | None:
    """Recommend dietary shifts toward lower-emission options."""
    current = data.diet
...
def _consumption_recommendation(amount: float) -> Recommendation | None:
    """Recommend reduction actions for general spending and household waste."""
    if amount <= 0:
        return None
    return Recommendation(
...
def generate_rule_based_insights(data: CarbonInput, result: FootprintResult) -> InsightsResponse:
    ...
    if total <= target:
        summary = (
            ...
            f"the sustainable target of {target / _KG_PER_TONNE:.1f} t. Keep it up, and lock in these habits."
        )
    else:
        over = round((total - target) / _KG_PER_TONNE, 2)
        summary = (
            f"Your estimated footprint is {result.total_annual_tonnes} t CO2e/yr, about {over} t "
            f"above the sustainable target of {target / _KG_PER_TONNE:.1f} t. The actions below target your "
