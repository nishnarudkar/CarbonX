...
describe("ResultBreakdown", () => {
  ...
  it("renders with 'under' class when under target", () => {
    const underResult = {
      ...result,
      comparison: {
        ...result.comparison,
    ...
    const totalEl = screen.getByText(/5 t CO₂e/i);
    expect(totalEl).toHaveClass("under");
    expect(totalEl).not.toHaveClass("over");
  });
});
